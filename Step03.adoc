...
....
    ...
    Checkpoint authenticationCheckpoint = testContext.checkpoint();   <3>
....
<1> Create the WebClient to call our endpoint
<2> <3> Create the Checkpoints so that the test exits
